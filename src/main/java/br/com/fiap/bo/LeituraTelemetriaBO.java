...
public class LeituraTelemetriaBO {
    ...
    public LeituraTelemetria registrar(LeituraTelemetria leitura) {
        if (leitura.getIdEstacao() <= 0)
            throw new IllegalArgumentException("ID da estacao invalido.");

        if (leitura.getSst() < -5 || leitura.getSst() > 50)
            throw new IllegalArgumentException("SST invalida. Deve estar entre -5 e 50 C.");

        if (leitura.getWaveHeight() < 0 || leitura.getWaveHeight() > 50)
            throw new IllegalArgumentException("Altura de onda invalida. Deve estar entre 0 e 50 m.");

        if (leitura.getEarthquakeMagnitude() < 0 || leitura.getEarthquakeMagnitude() > 10)
            throw new IllegalArgumentException("Magnitude invalida. Deve estar entre 0 e 10.");

        if (leitura.getFocalDepth() < 0)
            throw new IllegalArgumentException("Profundidade focal nao pode ser negativa.");

        dao.inserir(leitura);

        // gera alerta automatico se necessario
    ...
    public LeituraTelemetria buscarPorId(int id) {
        ...
        if (leitura == null)
            throw new IllegalArgumentException("Leitura com ID " + id + " nao encontrada.");
        return leitura;
    }
